...
void ContrastiveLossLayer<Dtype>::Forward_gpu(
  ...
  bool legacy_version =
      this->layer_param_.contrastive_loss_param().legacy_version();// will be unused
  Dtype loss(0.0);
...
void ContrastiveLossLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    if (static_cast<int>(bottom[2]->cpu_data()[i])) {  // similar pairs
      //std::cout << "similar pairs"  << "\n";
      loss += dist_sq_.cpu_data()[i];
    } else {  // dissimilar pairs
      //std::cout << "dissimilar pairs" << "\n";
      loss += std::max(margin - diff_.cpu_data()[i], Dtype(0.0));// change by YangXS: dist_sq_ to diff_
